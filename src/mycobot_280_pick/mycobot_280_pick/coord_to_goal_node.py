...
#
# ⚠️ [2026-07-30] 이 값을 바꾸면 **같이 고쳐야 하는 곳이 5군데** 있다:
...
#   5. [2026-08-02 추가] **WORLD_UP** — roll 0을 look pose 그리퍼 방향에
#      맞춰 둔 값이라, look pose가 바뀌면 부호가 맞는지 다시 재야 한다.
#
# 1~3은 어긋나면 증상이 바로 보이지만(RViz 자세 불일치, 검출이 안 열림 등),
# **4와 5는 에러가 전혀 안 난다** — 4는 접근 각도만 조금씩 틀어지고, 5는
# 그리퍼가 뒤집힌 채로 정상 동작한다. 그래서 팔이 look pose에 도착할 때마다
# TF로 자동 대조하도록 해뒀다(_verify_approach_reference_point) — 4는 어긋나면
# 로그에 경고가 뜬다. 5는 자동 검사가 없으니 눈으로 확인할 것.
...
#
# 어떻게 정했나 — 그리고 1차 시도가 왜 위험했나
# ---------------------------------------------
# [폐기된 1차 시도] 15개 목표의 정렬 관절해를 모아 **관절공간 minimax 중심**을
# 구했다(scripts/find_armed_pose.py). 결과는 (+27.08, -2.39, -63.53, +1.70,
# -80.44, +64.83)도였는데, FK로 확인하니 flange가 (+0.194, -0.024, +0.306) —
# 즉 **베드 쪽으로 뻗어 가장 가까운 토마토까지 7.3cm**였다. 그리퍼가 flange에서
# 9cm 더 나가므로 손가락은 이미 열매 사이에 있는 자세다(사용자가 RViz에서 발견).
# 원인이 둘이다:
#   (1) 목적함수 — "모든 목표까지 관절 이동량 최소"는 제약이 없으면 **목표
#       한가운데로 파고드는 것**이 최적해다.
#   (2) 그 측정에는 베드도 토마토도 collision object가 없어서, 뚫고 가는 경로가
#       전부 성공으로 집계됐다.
#
# [채택된 2차 시도] scripts/find_armed_pose_cartesian.py
#   - 토마토 15개를 구 collision object로 씬에 넣고,
#   - ACM으로 그리퍼/손목/flange만 토마토 충돌을 허용한다(팔뚝은 그대로 막힘).
#     이걸 빼면 접근할 토마토 자신과 충돌해 성공률이 2~4%로 떨어진다 — 노드가
#     _allow_gripper_target_object_collision으로 하는 것과 같은 처리다.
#   - **작업공간 안전 조건**으로 거른다: 그리퍼 끝단이 어느 토마토에서든 8cm 이상.
#
# 여기서 구조적인 사실이 드러났다. "그리퍼가 베드를 향하는" 후보는 끝단이
# 반경+9cm가 되어 토마토(반경 0.248~0.293) 한가운데로 들어간다 — 작업공간 격자
# 36개 중 34개가 여유 부족으로 탈락했다. look pose가 안전한 이유는 flange
# 방위각이 **-165.7도**, 즉 팔이 **베이스 뒤로 접혀** 있고 카메라만 앞을 보기
# 때문이다. **안전한 대기 자세는 "뻗은" 것이 아니라 "접힌" 자세다.**
#
# 그래서 look pose의 접힌 기하를 유지한 채 J1(과 J2/J3)만 바꾼 후보 21개를
# 평가했고, 결론은 **look pose에서 J1만 +40도 돌린 것**이다. J2~J6는 손댈 이유가
# 없었다.
#
# 실측 (15개 x 3회, 토마토 collision object 포함, 방향 허용오차 0.2rad)
#   끝단 여유       27.0cm      (1차 시도는 7.3cm)
#   flange          반경 0.141m, 방위 약 -166도(베드 반대편)
#   플래닝 성공률   15/15 목표 100%
#   이동량          중앙 502도, 최악 566도
#
# 주의 1: 이전에 여기 적혀 있던 "왕복 -26%"는 **토마토가 씬에 없을 때** 잰
#   값이라 낙관적이었다. 토마토를 넣으면 편도가 452 -> 502도로 늘어난다.
# 주의 2: 씬에 넣은 것은 열매뿐이다. 줄기·지지대·잎은 형상을 몰라 빠져 있어
#   이 수치도 여전히 낙관적이다. 실물 전에 녹화 장면의 octomap으로 재확인할 것.
# 주의 3: 이 값은 bags/lab_bed_detections.json의 토마토 분포에 맞춘 값이다.
#   베드나 카메라 자세가 크게 바뀌면 다시 도출해야 한다.
# 주의 4: 이 자세에서는 카메라가 베드를 보지 않는다. YOLO 판단 게이팅이 안
#   열리는데, 시퀀스 진행 중에는 그게 오히려 안전하다(자동 재트리거 방지).
#   대신 **다음 스냅샷 전에는 반드시 look pose로 한 번 가야 한다.**
#
# 참고: 목표마다 armed pose를 다르게 하면(docs/new_concept.md 옵션2) 이동량이
#   중앙 399도(-20.6%)로 더 준다. 다만 서로 다른 자세가 9개 필요해 "대기 자세"
#   라는 개념이 옅어지고 구조가 복잡해져, 우선 단일 자세를 채택했다.
ARMED_POSE_JOINT_POSITIONS = [
    0.698132,    # +40.00도  <- look pose(-7.47도)에서 이 축만 돌린 것
    1.816190,    # +104.06도 ┐
    -0.989253,   #  -56.68도 │ look pose와 동일 — 팔을 뒤로 접은 이 기하가
    -0.872665,   #  -50.00도 │ 베드와의 여유(27cm)를 만든다
    0.279078,    #  +15.99도 │
    0.024435,    #   +1.40도 ┘
]
...
# 향후 AI Service가 토마토 기울기 정보를 주면 이 값을 그 기울기 벡터로 교체할
# 예정(docs/obstacle_avoidance_manual_test.md "향후 계획" 참고).
#
# [2026-08-02] world +Z에서 **-Z로 뒤집었다.** 이 값은 "위쪽"이라는 물리적
# 의미가 아니라 **roll 0을 어디에 둘 것인가**를 정하는 기준일 뿐인데, +Z로
# 두면 roll 0이 look pose 그리퍼와 정확히 반대가 된다.
#
# 근거(실측): look pose flange의 실제 orientation
# (FALLBACK_APPROACH_QUAT_XYZW)에서 forward(로컬 +Z)와 right(로컬 +X)를 뽑아,
# 같은 forward에 대해 WORLD_UP=+Z 규약이 만드는 right와 비교하면
#     roll 차이 = -177.9도
# 즉 **roll=0이 곧 "뒤집힌 자세"**였다. 실측 토마토 15개 전부에서 look pose
# 그리퍼 방향에 가까운 쪽이 roll 0(편차 157~177도)이 아니라 roll 180
# (편차 3~23도)이었다.
#
# 증상: RViz 스윕에서 목표로 진입할 때 flange가 자기 축 둘레로 반 바퀴 도는
# 목표가 다수 있었다(J6 자전량 중앙 166도). roll 후보 탐색이 IK 실패로
# 전멸하면 `roll 0` 강행 폴백(_try_next_roll_candidate 말미)으로 빠지는데,
# 그 폴백이 하필 뒤집힌 쪽이었던 것.
#
# 왜 -Z로 충분한가: _compute_look_at_quat_xyzw에서 right0=cross(up,forward),
# up0=cross(forward,right0)이므로 up_ref의 부호를 뒤집으면 right0/up0가 함께
# 뒤집혀 **roll을 180도 돌린 것과 정확히 같다.** ROLL_CANDIDATES_RAD는 30도
# 간격 12개라 180도 짝을 이미 포함하므로 **탐색 집합 자체는 변하지 않고**,
# 바뀌는 것은 roll 0의 의미와 탐색 순서, 그리고 위 강행 폴백뿐이다.
#
# **look pose를 바꾸면 이 값을 반드시 재확인할 것** (LOOK_POSE_JOINT_POSITIONS
# 주석의 결합 목록 5번). 어긋나도 에러가 안 나고 그리퍼만 뒤집힌 채 돈다.
...
#
# [2026-08-02, 5차] 0.02 -> 0.03.
#
# 왜 늘리나: 직진 접근 구간이 곧 "주변 열매를 안 건드리고 들어가는 구간"이다.
# 2cm는 정렬 위치가 이미 열매 코앞이라는 뜻이고, 정렬(1/5)은 OMPL 자유공간
# 경로라 어느 방향에서 들어올지 보장이 없다. 길수록 OMPL이 끝나는 지점이
# 열매에서 멀어지고, 마지막 구간은 접근축을 따라 직선임이 보장된다.
# (사용자 요구: "목표 50mm 앞에 도착한 뒤 진짜 목표까지 직진")
#
# **왜 50mm가 아니라 30mm인가 — 이게 이 값의 핵심이다.**
# 정렬 반경 = 목표 반경 - (그리퍼 길이 + standoff)·cos(고도각) 이므로,
# standoff를 키우면 정렬 위치가 베이스 쪽으로 당겨져 MIN_ALIGN_RADIUS_M(0.13)에
# 걸린다. 걸리면 탐색은 실패하는 대신 **더 급한 고도각으로 도망간다** — 즉
# "정면으로 들어가려고 늘린 값이 오히려 접근을 비스듬하게 만든다."
#
# 실측(목표 15개, bags/lab_bed_detections.json, 기하 게이트만):
#     standoff  게이트   통과    채택 고도각   |고도각|평균  수직우회  정렬반경
#      20mm     0.130   15/15   -10~+15도      8.7도      17mm  0.140~0.184
#      30mm     0.130   15/15   -10~+15도      8.7도      18mm  0.130~0.175
#      40mm     0.130   15/15   -25~+20도     12.7도      28mm  0.131~0.165
#      50mm     0.130   15/15   -35~+30도     20.0도      47mm  0.131~0.155
#      50mm     0.110   15/15   -10~+15도      8.7도      21mm  0.111~0.155
#
# **30mm까지는 채택 고도각이 20mm와 완전히 같다(-10~+15도).** 즉 30mm는
# 접근 방향을 하나도 바꾸지 않으면서 직진 구간만 1.5배로 늘리는 순수 이득이다.
# 40mm부터 고도각이 기울기 시작하고, 50mm에서는 우회량이 17->47mm로 튄다.
#
# 50mm를 쓰려면 MIN_ALIGN_RADIUS_M을 0.11로 낮춰야 하고, 그러면 손목 오프셋
# 관계식 asin(0.0732/r)상 필요 J1이 34도 -> 41도로 늘어난다(2026-07-30
# 리팩터가 57도 -> 34도로 줄여 놓은 값). 그 대가를 치를지는 별도 판단이며,
# 여기서는 **대가 없는 30mm**를 택했다.
#
# 확인할 것: 직진 구간이 1.5배 길어지므로 Cartesian 보간이
# CARTESIAN_FRACTION_THRESHOLD(0.95)를 못 넘을 수 있다. 못 넘으면
# _verify_grasp_approach_reachable 사전 dry-run에서 걸려 정렬 재시도가 는다.
APPROACH_STANDOFF_M = 0.03 # 0.02 # 0.05 # 0.08
...
class CoordToGoalNode(Node):
    ...
    def _build_approach_elevation_candidates(self):
        ...
        [2026-08-02 정정] 이 자리에 *"목록은 |고도각| 오름차순이므로 수평
        접근(0도)이 먼저 쓰인다"*고 적혀 있었으나 **2026-07-30 4차 개정 이후로
        사실이 아니다.** 지금 정렬 기준은 `elevation_error_rad`, 즉 **이상
        고도각(APPROACH_REFERENCE_POINT -> 목표)에 가까운 순**이다(아래 sort).
        0도가 특별 취급되지 않으므로, APPROACH_STANDOFF_M이 커져 0도가 게이트에
        걸려 사라져도 프로파일은 그대로 매끄럽다(5차 개정 근거).
        """
        target = Point()
    ...
    def _try_next_roll_candidate(self) -> None:
        ...
        if self._roll_search_index >= len(ROLL_CANDIDATES_RAD):
            ...
            # 골랐음(사용자 관측). 완만한 티어에서 멈추는 것이 곧 목적함수를
            # 따르는 것임. (1)의 함정(성급한 채택)은 티어 안에 비교 대상이
            # 충분해 완화됨 — 그래도 로그의 J1 값으로 재확인할 것.
            #
            # [2026-08-02 정정] 위 3차 주석이 목적함수를 "수직 우회량 최소"라고
            # 적어 두었으나 **코드와 다르다.** 4차 개정에서 1차 정렬 기준이
            # `elevation_error_rad`(이상 고도각과의 차이)로 바뀌었고,
            # `vertical_detour_m`은 후보 dict에 담겨 **로그로만 나갈 뿐 정렬에
            # 전혀 쓰이지 않는다.** 둘은 대체로 같은 방향이지만 같은 값이 아니다.
            finished = self._approach_candidates[self._approach_candidate_index]
            self._approach_candidate_index += 1
            self._roll_search_index = 0
            self._roll_search_retry = 0

            # [2026-07-30, 3차] 같은 티어를 다 훑기 전에는 채택하지 않고, 티어
            # 경계에서 해가 있으면 그 자리에서 채택함. 전역 훑기(모든 고도각을
            # 다 보고 관절 이동량 최소를 고르는 방식)에서 이렇게 바꾼 이유는
            # 위 정렬 기준 주석 참고.
            #
            # 티어 = 이상 고도각에서 APPROACH_ELEVATION_TIER_RAD 안쪽인 후보
            # 묶음. 고도각 하나만 보고 성급히 채택했을 때 걸렸던 함정(J1이
            # 150도 도는 IK 분기를 그대로 집는 문제)을 피할 만큼 비교 대상을
            # 확보하는 것이 목적이다 — 그래도 로그의 J1 값으로 확인할 것.
            #
            # [2026-08-02 정정] 위 주석이 "티어 10도, 티어당 후보 3~4개 ×
            # roll 12 = 36~48개"라고 적고 있었으나 **현재 값은 5도**다
            # (APPROACH_ELEVATION_TIER_RAD, 6차 개정에서 10 -> 5). 5도 간격
            # 사다리에 5도 티어면 티어당 후보는 1~2개 × roll 12 = 12~24개다.
            # 티어를 더 줄이면 후보가 1개(roll 12개)로 떨어져 위 함정에 다시
            # 노출되므로 5도가 하한이다.
            next_candidate = (
    ...
    def _start_bin_release(self) -> None:
        ...
        **접근 때와 같은 폭**(대상 반지름 기준, [2/5]와 같은 계산)으로 연다.
        근거는 "충분히 넓을 것 같아서"가 아니라 **그 폭으로 이미 통과한 열매**라는
        것이다 — [3/5] 직진 접근에서 손가락이 그 간격으로 열매를 지나 감쌌으므로,
        같은 값으로 되돌리면 반드시 빠진다.

        최대 개방(GRIPPER_OPEN_POSITION, 폭 약 50mm)으로 벌리지 않는 이유: 통
        입구가 8cm인데 이 자세의 끝단이 통 중심에서 16mm 어긋나 있어(10절),
        손가락 끝이 ±25mm까지 벌어지면 림에 닿을 수 있다. 접근 폭이면 ±17.5mm라
        여유가 있다. **통 실물 치수는 아직 확인 전이므로**(7절 2번) 넓히려면
        그 확인이 먼저다.
        """
