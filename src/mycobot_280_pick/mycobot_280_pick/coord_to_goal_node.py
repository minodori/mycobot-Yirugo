...
#
# [2026-08-03 밤, 실물] -0.49 -> **0.0**.
#
# 이 값은 "URDF 순기구학 근사 곡선을 스케일링해 역산" 한 것이지 실측이 아니었고
# (위 주석이 그렇게 밝히고 있다), 실물에서 과하게 좁은 것으로 확인됐다:
#   radius 12.5mm -> -0.600rad = **완전 닫힘**   (25mm 열매를 받을 공간이 0)
#   radius 17.4mm -> +0.053rad = 최대의 절반도 안 됨
# 오늘 파지가 연달아 실패한 목표들이 전부 작은 radius 쪽이었다. 좁게 여는
# 설계 의도(줄기·이웃이 같이 잡히는 걸 줄임)는 맞지만, **열매가 못 들어가면
# 의도가 무의미하다.**
#
# 0.0으로 올리면 매핑이 이렇게 바뀐다:
#   12.5mm  -0.600 -> **0.000**
#   17.4mm  +0.053 -> **+0.107**
#   18.0mm  +0.120 -> +0.120 (변화 없음, 최대)
# 즉 작은 열매일수록 개선 폭이 크고, 큰 열매는 그대로다.
#
# ⚠️ 여전히 근사값이다. 실물에서 개방폭(mm)을 관절값별로 재서 이 구간을
# 실측으로 바꾸는 것이 남은 일이다.
GRIPPER_OPEN_POSITION_SMALL = 0.0 # -0.49 (근사 역산값, 실물에서 과하게 좁았음)
...
# [2026-08-03] 사용자 재실측으로 위 90/110mm이 확인됨 — 이 값은 **파지 targeting
# 전용이고 맞는 값이다.** 첫 실물 시험 핸드오프가 "끝단이 목표보다 +30mm 더
# 들어가니 0.12로 키워야 한다"고 적었는데, 그건 이 상수의 오차가 아니라 닫는
# 동작 자체다 — 접근이 끝난 시점(열림, 끝단 90mm)에 끝단이 열매에 닿아 있고,
# 거기서 손가락이 오므라들며 110mm로 뻗으므로 끝단은 반드시 열매를 20mm 지나간다.
# 그래야 손가락이 열매 뒤쪽을 감싼다. 즉 관측된 +30mm 중 20mm는 정상 기하다.
# 이 값을 키우면 파지점이 손가락 뿌리 쪽으로 밀려 오히려 놓친다. 키우지 말 것.
# [2026-08-03 저녁, 실물 재관측 — 위 "20mm는 정상 기하"는 **틀렸다**]
# 그 논리는 "닫히면서 끝단이 90 -> 110mm로 뻗으니 열매를 20mm 지나가는 게
# 정상"이었다. 그런데 **적응형 그리퍼는 물체에 닿으면 거기서 멈춘다** — 자유
# 상태로 닫힐 때만 110mm까지 뻗지, 열매를 물면 그 전에 선다. 즉 그 20mm는
# 일어나지 않고, 관측된 초과는 대부분 진짜 오차다.
#
# 실물에서 두 세션 연속 같은 방향으로 관측됐다("그리퍼가 토마토보다 더
# 들어감"). 위 조정 규칙 그대로 값을 키운다: 0.09 -> 0.11 (사용자 판단 20mm).
# 0.11은 실측 닫힌 끝단 길이(GRIPPER_TIP_CLOSED_M)와 같은 값이다.
#
# [2026-08-03 저녁, 2차 실물] 0.11도 부족했다 — 여전히 깊이 들어간다는
# 실물 관측으로 0.12로 올린다. 0.09 기준 **총 30mm**이고, 이는 첫 실물 시험
# 때 기록된 "+30mm 더 들어간다"와 정확히 일치한다. 그 관측이 옳았던 것이다.
#
# 0.12는 실측 닫힌 끝단(110mm)보다 10mm 크다. 즉 flange를 손가락 길이보다
# 더 뒤로 뺀다 = 열매가 손가락 끝이 아니라 **패드 안쪽**에 앉는다. 감싸서
# 무는 자세로는 이쪽이 맞다.
#
# ⚠️ 대가: 정렬 위치가 0.09 대비 30mm 베이스 쪽으로 당겨진다.
#   반지름 0.291 목표: 정렬반지름 0.160 -> 0.142 (여유 +12mm)
#   반지름 0.325 목표: 정렬반지름 0.190 -> 0.176 (여유 +46mm)
# 가까운 목표(반지름 0.27 이하)는 하한 0.13에 붙기 시작한다. 여기서 더
# 키워야 한다면 값을 올리지 말고 **고도각을 기울여** 당기는 방향을 정면에서
# 벗어나게 하는 쪽을 먼저 볼 것(0.15로 올렸을 때 사각지대 재현 기록 있음).
# [2026-08-03 밤, 3차 실물 — 0.12는 **너무 멀다**]
# 0.12로 목표 2개를 돌렸더니 **둘 다 못 잡았다.** 기하가 설명한다:
#   flange -> 열매 = 120mm
#   손가락 끝 도달 = 90mm(열림) ~ 110mm(닫힘)
#   -> 닫아도 열매보다 10mm 짧다 = 열매가 손가락 범위 **밖**이다
# 실제로 넓게 연 목표(반지름 17.9mm, +0.109rad, 끝단 90mm)는 30mm 모자라
# 완전히 헛쳤고, 거의 닫힌 목표(12.5mm, -0.490rad, 끝단 110mm)는 10mm
# 모자라 끝에 **끼었다**(감싼 것이 아니다).
#
# 즉 이 상수의 상한은 닫힌 끝단 길이(0.11)이고, 열매가 손가락 사이에 들어
# 앉으려면 그보다 작아야 한다. 0.09(깊다)와 0.12(짧다) 사이인 **0.10**으로
# 간다. 0.09에서 "지나친다"고 본 것도 손가락 끝이 열매를 지나는 정상 기하를
# 일부 포함했을 가능성이 있으므로, 0.10에서 다시 눈으로 판정할 것.
#
# 다음에 조정할 때 반드시 볼 것: **그리퍼 열림 폭에 따라 도달거리가 20mm
# 달라진다.** 이 상수 하나로는 그 의존성을 못 담는다. radius_m이 작아
# 조금만 열리는 목표는 끝단이 멀리(110mm) 가고, 넓게 여는 목표는 가깝다
# (90mm). 근본적으로는 _gripper_open_position_for_radius와 이 값을 같이
# 푸는 것이 맞다.
# [2026-08-03 밤, TCP 실측 — 드디어 근거가 생겼다]
# 값을 눈으로 더듬는 것을 그만두고 **비전을 안 거치는 방법**으로 쟀다.
# 고정점 하나를 서로 다른 팔 자세로 찌르고 get_angles()를 읽으면, 끝단의
# g_base 위치는 모든 자세에서 같아야 하므로
#     (R_i - R_j) t = o_j - o_i
# 를 최소제곱으로 풀어 t(flange 기준 끝단 오프셋)를 얻는다. 관절 엔코더와
# URDF 기구학만 쓰므로 카메라/캘리브레이션 오차가 안 들어간다.
#
# 세 방법이 같은 영역으로 수렴했다:
#     자로 잰 닫힌 끝단      110mm
#     1차 정합(5자세)        111mm
#     2차 정합(7자세)        119mm
# 채택 0.115 (두 정합의 중간). 오늘 0.09 -> 0.11 -> 0.12 -> 0.10으로 네 번
# 옮긴 것은 전부 추측이었고, 이 값이 첫 실측 근거다.
#
# 정합은 옆쪽 치우침(tx, ty)도 4~9mm로 내놓았다. **스칼라 하나로는 표현할
# 수 없는 성분**이라 지금 구조로는 못 담는다. 다만 두 세션이 8.5/4.4mm로
# 갈려 아직 신뢰도가 낮으므로 반영하지 않는다. 이 값이 확실해지면
# _forward_unit_vector 기반 계산을 3차원 오프셋으로 바꿔야 한다.
#
# 잔차 RMS 9.2mm는 이 값의 불확실성이 아니라 자세마다 찌른 지점이 흩어진
# 정도다. 접근축이 전부 아래를 향해(z 성분 -0.41 ~ -0.97) tz가 한쪽으로만
# 구속된 탓인데, **이 팔은 베드를 수평으로 찌를 수 없어**(어깨 도달 한계)
# 그 이상의 다양성을 못 만든다. 실제 수확이 그 범위에서만 일어나므로
# 용도에는 맞는 데이터다.
# [2026-08-03 밤, 정정] TCP 정합값 115mm는 **손가락 최끝단**까지의 거리다.
# 그 측정은 접촉점을 고정하려고 케이블 타이로 손가락 끝을 묶고 했다 —
# 방법으로는 옳지만(강체가 되어 (R_i-R_j)t=o_j-o_i 가정이 정확히 성립),
# 묶인 지점은 손가락 둘이 모인 **끝**이지 열매가 앉는 **패드**가 아니다.
#
#   flange ────────── 패드(파지점) ──── 최끝단(묶인 곳)
#                     ↑ 이 상수가 가리켜야 할 곳    ↑ TCP 115mm
#
# 115를 그대로 넣었더니 flange를 과하게 뒤로 빼서 정렬 위치가 사각지대에
# 빠졌다(반지름 0.242 목표에서 정렬 0.097m, 하한 0.13에 33mm 미달로 고도각
# -35~+35도가 전부 탈락). 최끝단에서 열매 반지름(18mm)만큼 안쪽이 파지점이므로
# 약 0.10이 맞다.
#
# ⚠️ 오늘의 파지 판정(0.09 -> 0.11 -> 0.12 -> 0.10 -> 0.115)은 **전부 무효**다.
# 그 실행들이 모두 케이블 타이가 묶인 채로 돌아서 그리퍼가 열리지 않았고,
# "깊다/얕다"가 못 무는 상태에서의 관측이었다. 타이를 푼 뒤 다시 판정할 것.
GRIPPER_LENGTH_OFFSET_M = 0.10 # 최끝단 115mm - 열매 반지름 ≈ 파지점. 위 주석 필독

# 닫힌 상태의 flange -> 손가락 끝 거리(m) — 위 실측 110mm.
# targeting이 아니라 **여유(clearance) 계산 전용**이다. 열매를 쥐고 이동하는
# 구간(통 자세 경유·복귀)에서 그리퍼는 닫혀 있으므로, 끝단이 어디까지 뻗는지를
# 재는 코드는 GRIPPER_LENGTH_OFFSET_M이 아니라 이 값을 써야 한다. 둘을 섞으면
# 여유를 20mm 과대평가한다(통 놓기 높이가 실제로는 림 위 29mm가 아니라 9mm).
GRIPPER_TIP_CLOSED_M = 0.11
...
# 물체가 떨어지거나 흔들리지 않게 함.
# [2026-08-03, 실물] 0.2 -> 0.3. 이 구간에서 팔이 "건들건들" 떨었다.
# SPEED_TUNING_HANDOFF가 같은 표현으로 기록한 현상과 원인이 같을 수 있다 —
# 서보가 명령마다 목표에 **일찍 도착해 멈췄다 재출발**하면 초당 십수 번의
# 미세 떨림이 된다. 로컬 스케일링이 낮을수록 명령당 이동량이 작아져 그
# 조건에 가까워진다. 올려서 명령당 이동량을 키운다.
RETREAT_VELOCITY_SCALING = 0.3 # 0.2 #0.05
...
# [2026-08-03, 실물] 0.2 -> 0.3. 근거는 RETREAT_VELOCITY_SCALING 주석과 같다.
# 두 값을 같이 올리는 이유는 두 구간이 같은 Cartesian 경로를 반대로 오가는
# 것이라 한쪽만 바꾸면 왕복이 비대칭이 되기 때문이다.
APPROACH_VELOCITY_SCALING = 0.3 # 0.2 #0.1 #0.05
...
class CoordToGoalNode(Node):
    ...
    def _on_target_point(self, msg: PointStamped) -> None:
        ...
        # [2026-08-03] 손에 열매가 있으면 새 목표를 받지 않는다.
        #
        # 이건 상위 시퀀스 정지(_pending_result_succeeded = False)와 **별개의
        # 방어선**이다. 시퀀스를 거치지 않고 좌표가 직접 들어오는 경로가
        # 여럿 있고(수동 발행, 재시작, publish_target_point), 그 경우 열매를
        # 쥔 채 정렬로 들어가 [2/5]가 **베드 위에서** 그리퍼를 연다.
        # 실물 사고가 난 자리가 정확히 "열매를 쥔 채 다음 목표로 이동"이라,
        # 그 상태를 만드는 입구를 하나 더 막는다.
        if self._holding_object:
            self.get_logger().error(
                '그리퍼가 열매를 쥔 채라 새 좌표를 거부한다 — 이 상태로 '
                '정렬에 들어가면 [2/5]가 베드 위에서 열매를 놓는다. '
                '손에서 열매를 빼고 /go_to_look_pose로 정렬한 뒤 다시 보낼 것.')
            return
        ...
        # [2026-08-03, 실물] octomap을 **여기서** 지운다.
        #
        # 예전엔 `_continue_planning_from_waiting_pose`에서만 지웠는데, 그건
        # 통 자세에 **도착한 뒤**라 [0/5] 경유는 살아 있는 octomap을 상대로
        # 계획했다. 그래서 구간마다 보는 씬이 달랐다:
        #   [0/5] 경유 · 복귀 -> 살아 있는 voxel
        #   [1/5] 정렬        -> 지워진 씬
        # 실물에서 이것 때문에 시퀀스가 첫 이동에서 죽었다 — 출발 상태의
        # joint3가 voxel과 충돌이라 OMPL이 18ms 만에 FAILURE를 냈다(탐색조차
        # 못 함). 정렬만 통과하고 경유·복귀가 골라 죽던 지난 세션 증상도
        # 같은 뿌리다.
        #
        # look_pose_gate로는 이걸 못 막는다 — 게이트는 "look pose에 있을 때만
        # 클라우드를 낸다"인데 [0/5]는 **바로 그 look pose에서 출발**한다.
        #
        # 아래 대기가 이미 있으므로 지우기와 플래닝 사이 간격도 확보된다.
        if self._clear_octomap_client.service_is_ready():
            self._clear_octomap_client.call_async(Empty.Request())
            self.get_logger().info('사이클 시작 — octomap 클리어(경유 포함 전 구간 동일 조건)')
    ...
    def _check_bin_release_complete(self) -> None:
        ...
        if self._gripper_moveit2.motion_suceeded:
            ...
            # [2026-08-03] 여기도 경고만 하고 진행하고 있었다 — 복귀 실패
            # (위 _pending_result_succeeded = False)와 **같은 사고의 한 단계
            # 뒤 판본**이다. 놓기에 실패하면 열매를 쥔 채로 다음 사이클에
            # 들어가고, 다음 사이클의 [2/5] 그리퍼 열기가 **정렬 위치에서**
            # 열어 버린다 = 베드 위에 떨어뜨린다. 그 전에 열매를 쥔 채
            # 베드를 가로지르는 것도 그대로다(이웃 열매는 장애물이 아니다,
            # 함정 17).
            #
            # 주석이 "수동 확인 필요"라고 적혀 있었지만 멈추는 코드가 없어서
            # 아무도 확인할 틈이 없었다. 복귀 실패와 똑같이 실패로 보고해
            # 상위 시퀀스가 멈추게 한다.
            self._pending_result_succeeded = False
            self.get_logger().error(
                '[통] 놓기 실패 — 그리퍼가 열매를 쥔 채다. 이 사이클을 '
                '실패로 보고해 시퀀스를 멈춘다. 손에서 열매를 빼고 '
                '/go_to_look_pose로 정렬한 뒤 다시 시작할 것.'
            )
