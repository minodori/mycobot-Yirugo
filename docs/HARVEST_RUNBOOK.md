...
>
> 2026-08-03 기준. **첫 실물 시험을 마쳤다** — 파지 5단계는 성공, 전이는 무너졌다.
> 무엇이 왜 갈렸는지는 **5.2b**, 다음에 이어서 할 일은
> `docs/SESSION_HANDOFF_20260803.md`에 있다.
...
배수가 걸리면 로그에 찍힌다 — `[1/5 정렬] 속도 1.00/가속 0.50 (speed_scale 5.0배)`.
**안 찍히면 안 걸린 것이다**(그게 "배수를 올려도 차이가 없다"의 신호다).
...
> **[2026-08-03 정정] `sync_plan`은 단방향이 아니다.** `/joint_states`를 되돌려
> 발행한다(`control_sync_plan`이 퍼블리셔로 잡힌다). 즉 실물 관절각이 ROS로
> 들어오는 경로가 이미 있다.
>
> 그리고 **리포커스 전에 시뮬을 실물 자세에 맞춰야 한다** —
> `refocus_servos`는 실물 현재 각도로 토크를 잠그지만, relay가 재개되는 순간
> 다음 `/joint_states`(= 시뮬)로 팔이 **한 번에** 간다. 그 서비스의 응답 메시지가
> 그렇게 경고하는데 RPi 로그에만 남아 로컬에서는 안 보인다.
...
### 5.2b 첫 실물 시험에서 나온 것 (2026-08-03)

목표 2개(`target_source:=file`)로 처음 돌렸다. **첫 목표는 5단계 전부 성공**했고
(정렬 → 그리퍼 → 직진 → 파지 → 후퇴), 그 뒤가 갈렸다.

| 관측 | 원인 | 상태 |
|---|---|---|
| **그리퍼 끝단이 목표보다 +30mm 더 들어감**(g_base +x) | `GRIPPER_LENGTH_OFFSET_M = 0.09`가 실제보다 짧을 가능성 — 30mm 초과면 실제 flange→끝단이 **약 0.12m** | **실측 필요**(자로 flange 면에서 손가락 끝까지) |
| 통 자세 복귀·경유·look pose 이동이 **전부 실패** | `Motion plan was found but it seems to be invalid` — 실물 카메라가 octomap을 계속 갱신해, 계획 직후 씬이 바뀌어 재검증에서 충돌 판정. 짧은 구간(정렬·직진)은 `/clear_octomap` 직후라 통과했고 **긴 전이만 골라 실패**했다 | `look_pose_gate:=true`로 막는다(4.4절) |
| 복귀 실패 후 **다음 목표로 진행**하며 베드의 이웃 토마토와 충돌 | 경고만 하고 넘어갔다. 게다가 이웃 열매는 장애물이 아니다(핸드오프 함정 17) | **고침** — 복귀 실패를 실패로 보고하고 시퀀스가 큐를 버리고 멈춘다 |

> **복귀 실패는 "팔이 어디 있는지 모른다"와 같은 뜻이다.** 그 상태에서 다음
> 목표로 가면 어떤 경로가 될지 아무도 모른다. 이제 그 지점에서 멈춘다.

**실물 세션 전에 `look_pose_gate`를 켤 것** — 위 두 번째 줄이 그것 하나로 막힌다:
...
- **`GRIPPER_LENGTH_OFFSET_M`의 실측값** — 첫 시험에서 30mm 어긋났다(위 5.2b)
